import { useTranslation } from 'react-i18next';

export default function Footer() {
  const { t } = useTranslation();

  return (
    <footer style={{ backgroundColor: '#0A1807', overflow: 'hidden', position: 'relative' }}>

      {/* Franja superior de colores */}
      <div style={{ height: '4px', background: 'linear-gradient(90deg, #F4E800, #E8173A, #1B7A2F, #F4E800)' }} />

      {/* Cuerpo principal */}
      <div className="max-w-7xl mx-auto px-6 sm:px-10 py-16 sm:py-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 lg:gap-16">

          {/* Columna 1 — Logo y tagline */}
          <div>
            <img
              src="/VERSIONES LOGO-02.png"
              alt="Productos BellaVista"
              style={{ height: '80px', width: 'auto', marginBottom: '20px', filter: 'brightness(0) invert(1)' }}
            />
            <p style={{
              fontFamily: 'Schoolbell, cursive',
              color: '#F4E800',
              fontSize: '18px',
              lineHeight: 1.4,
              marginBottom: '12px',
            }}>
              {t('footer.tagline')}
            </p>
            <p style={{
              fontFamily: 'Kumbh Sans, sans-serif',
              color: 'rgba(255,255,255,0.55)',
              fontSize: '13px',
              lineHeight: 1.75,
            }}>
              {t('footer.descripcion1')}
            </p>
            <p style={{
              fontFamily: 'Kumbh Sans, sans-serif',
              color: 'rgba(255,255,255,0.55)',
              fontSize: '13px',
              lineHeight: 1.75,
              marginTop: '8px',
            }}>
              {t('footer.descripcion2')}
            </p>
          </div>

          {/* Columna 2 — Contacto */}
          <div>
            <h4 style={{
              fontFamily: 'Kumbh Sans, sans-serif',
              color: '#F4E800',
              fontSize: '0.7rem',
              fontWeight: 700,
              letterSpacing: '0.3em',
              textTransform: 'uppercase',
              marginBottom: '20px',
            }}>
              {t('footer.contactoTitulo')}
            </h4>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {/* Email */}
              <a
                href="mailto:contactenos@panelabellavista.com"
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '10px',
                  color: 'rgba(255,255,255,0.75)',
                  fontFamily: 'Kumbh Sans, sans-serif',
                  fontSize: '13.5px',
                  textDecoration: 'none',
                  transition: 'color 0.2s',
                }}
                onMouseEnter={e => e.currentTarget.style.color = '#F4E800'}
                onMouseLeave={e => e.currentTarget.style.color = 'rgba(255,255,255,0.75)'}
              >
                <svg width="16" height="16" fill="currentColor" style={{ color: '#E8173A', flexShrink: 0 }} viewBox="0 0 20 20">
                  <path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z" />
                  <path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z" />
                </svg>
                contactenos@panelabellavista.com
              </a>

              {/* Ubicación */}
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                <svg width="16" height="16" fill="currentColor" style={{ color: '#E8173A', flexShrink: 0, marginTop: '2px' }} viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" />
                </svg>
                <span style={{
                  fontFamily: 'Kumbh Sans, sans-serif',
                  color: 'rgba(255,255,255,0.75)',
                  fontSize: '13.5px',
                  lineHeight: 1.5,
                }}>
                  {t('footer.ubicacion')}
                </span>
              </div>

              {/* WhatsApp */}
              <a
                href="https://wa.me/573184550936?text=Hola%2C%20me%20interesa%20cotizar%20productos%20Bellavista.%20%C2%BFMe%20pueden%20ayudar%3F"
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '10px',
                  color: 'rgba(255,255,255,0.75)',
                  fontFamily: 'Kumbh Sans, sans-serif',
                  fontSize: '13.5px',
                  textDecoration: 'none',
                  transition: 'color 0.2s',
                }}
                onMouseEnter={e => e.currentTarget.style.color = '#F4E800'}
                onMouseLeave={e => e.currentTarget.style.color = 'rgba(255,255,255,0.75)'}
              >
                <svg width="16" height="16" fill="currentColor" style={{ color: '#25D366', flexShrink: 0 }} viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347M12.051 21.785h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                +57 318 455 0936
              </a>
            </div>
          </div>

          {/* Columna 3 — Redes */}
          <div>
            <h4 style={{
              fontFamily: 'Kumbh Sans, sans-serif',
              color: '#F4E800',
              fontSize: '0.7rem',
              fontWeight: 700,
              letterSpacing: '0.3em',
              textTransform: 'uppercase',
              marginBottom: '20px',
            }}>
              {t('footer.redesTitulo')}
            </h4>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {/* Instagram */}
              <a
                href="https://instagram.com/productosbellavista"
                target="_blank"
                rel="noopener noreferrer"
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '10px',
                  color: 'rgba(255,255,255,0.75)',
                  fontFamily: 'Kumbh Sans, sans-serif',
                  fontSize: '13.5px',
                  textDecoration: 'none',
                  transition: 'color 0.2s',
                }}
                onMouseEnter={e => e.currentTarget.style.color = '#F4E800'}
                onMouseLeave={e => e.currentTarget.style.color = 'rgba(255,255,255,0.75)'}
              >
                <svg width="16" height="16" fill="currentColor" style={{ color: '#E4405F', flexShrink: 0 }} viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                </svg>
                @productosbellavista
              </a>

              {/* Facebook */}
              <a
                href="https://facebook.com/ProductosBellaVistaExport"
                target="_blank"
                rel="noopener noreferrer"
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '10px',
                  color: 'rgba(255,255,255,0.75)',
                  fontFamily: 'Kumbh Sans, sans-serif',
                  fontSize: '13.5px',
                  textDecoration: 'none',
                  transition: 'color 0.2s',
                }}
                onMouseEnter={e => e.currentTarget.style.color = '#F4E800'}
                onMouseLeave={e => e.currentTarget.style.color = 'rgba(255,255,255,0.75)'}
              >
                <svg width="16" height="16" fill="currentColor" style={{ color: '#1877F2', flexShrink: 0 }} viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
                Productos BellaVista Export
              </a>
            </div>

            {/* Separador y categorías */}
            <div style={{ marginTop: '28px', paddingTop: '20px', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
              <p style={{
                fontFamily: 'Kumbh Sans, sans-serif',
                color: 'rgba(255,255,255,0.4)',
                fontSize: '11px',
                letterSpacing: '0.2em',
                textTransform: 'uppercase',
                marginBottom: '10px',
              }}>
                {t('footer.categoriasTitulo')}
              </p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {['🌿 Panela', '☕ Café', '🍵 Bebidas'].map((cat, i) => (
                  <span key={i} style={{
                    backgroundColor: 'rgba(244,232,0,0.1)',
                    border: '1px solid rgba(244,232,0,0.25)',
                    color: '#F4E800',
                    fontFamily: 'Kumbh Sans, sans-serif',
                    fontSize: '11px',
                    fontWeight: 600,
                    padding: '4px 10px',
                    borderRadius: '4px',
                    letterSpacing: '0.05em',
                  }}>
                    {cat}
                  </span>
                ))}
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* Pie del footer */}
      <div style={{
        borderTop: '1px solid rgba(255,255,255,0.08)',
        padding: '16px 24px',
        display: 'flex',
        flexWrap: 'wrap',
        justifyContent: 'center',
        alignItems: 'center',
        gap: '12px',
        textAlign: 'center',
      }}>
        <span style={{
          fontFamily: 'Kumbh Sans, sans-serif',
          color: 'rgba(255,255,255,0.35)',
          fontSize: '12px',
        }}>
          © {new Date().getFullYear()} Productos BellaVista. {t('footer.derechos')}
        </span>
        <span style={{ color: 'rgba(255,255,255,0.15)', fontSize: '12px' }}>·</span>
        <span style={{
          fontFamily: 'Kumbh Sans, sans-serif',
          color: 'rgba(255,255,255,0.3)',
          fontSize: '12px',
        }}>
          Mogotes, Santander · Colombia 🇨🇴
        </span>
      </div>
    </footer>
  );
}
